...
python3 "/droid/repos/cl_shared/bb_sustained_telemetry.py" --check-active-hours | grep -q "ACTIVE HOURS" || exit 0

# Run full analysis and append to log
echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] Running sustained telemetry probe" >> "$LOG_FILE"
python3 "/droid/repos/cl_shared/bb_sustained_telemetry.py" >> "$LOG_FILE" 2>&1 || echo "[ERROR] Probe failed at $(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$LOG_FILE"
